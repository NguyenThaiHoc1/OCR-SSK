...
from ocr_package.ppocr.utils.utility import check_and_read
from ocr_package.ppocr.postprocess import build_post_process
from ocr_package.ppocr.data import create_operators, transform
from argparse import Namespace

import ocr_package.tools.infer.utility as utility
from ocr_package.tools.infer.utility import (
...
def draw_crop_rec_res(output_dir, img_crop_list):
    ...
    for bno in range(bbox_num):
        cv2.imwrite(
            os.path.join(
                output_dir, f"mg_crop_{bno + crop_image_res_index}.jpg"
            ),
            img_crop_list[bno],
        )
        # logger.debug(f"{bno}, vvvv")
    crop_image_res_index += bbox_num


def create_or_remove_folder(folder_name):
...
def main(args):
    ...
    print(args.crop_res_save_dir, "xxxx")
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        filename = remove_extension(image_file)
        crop_res_save_dir_file = os.path.join(args.crop_res_save_dir, filename)
        # os.makedirs(crop_res_save_dir_file, exist_ok=True)
        create_or_remove_folder(crop_res_save_dir_file)
        ...
        for index, img in enumerate(imgs):
            ...
            for bno in range(len(dt_boxes)):
                ...
                if args.det_box_type == "quad":
                    img_crop = get_rotate_crop_image(cp_img_ori, tmp_box)
                else:
                    img_crop = get_minarea_rect_crop(cp_img_ori, tmp_box)
                img_crop_list.append(img_crop)

            # crow
            if args.save_crop_res:
                draw_crop_rec_res(crop_res_save_dir_file, img_crop_list)
...
para_values = {
    ...
    'warmup': False,
    'draw_text_det_res': True,
    'save_crop_res': True,
    'crop_res_save_dir': './output_dir/text-det/crop_res_save_dir',
}
